...
title 'Factor Analysis with Factor Scores PUT into Plain Text File'; run;
DATA BEER;
...
PROC FACTOR METHOD=PRINIT REORDER ROTATE=VARIMAX
  PRIORS=SMC NFACTORS=2 RES OUT=DRINKME;
var cost size alcohol reputat color aroma taste; run;
****************************************************************************;
 *In the DATA step below we write a plain text data file with factor scores,
  SES, & Group for each subject that has complete data on the variables
  included in the factor analysis;
DATA put; SET drinkme; if factor1 ne . ;
FILE 'C:\Users\Vati\Documents\StatData\FactBeer.dat'; PUT factor1 factor2 ses group; run;
DATA alpha; SET drinkme;
NegRep = -1*reputat;
Proc Corr nomiss nosimple nocorr alpha; Var color taste aroma negrep; run;
